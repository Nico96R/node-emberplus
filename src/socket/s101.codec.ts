...
export enum S101CodecEvent {
    KEEP_ALIVE_REQUEST = 'keepAliveReq',
    EMBER_PACKET = 'emberPacket',
    KEEP_ALIVE_RESPONSE= 'keepAliveResp'
}

export class S101Codec extends EventEmitter {
    public inbuf: SmartBuffer;
    public emberbuf: SmartBuffer;
    public escaped: boolean;
    private pendingData: Buffer[];
    private isProcessing: boolean;

    constructor(private logger: LoggingService = new LoggingService(LogLevel.critical)) {
        super();
    ...
    encodeBER = function (data: Buffer): Buffer[] {
        const frames: Buffer[] = [];
    ...
    private processData(): Promise<any> {
    ...
    private handleEmberPacket(packet: SmartBuffer): void {
        this?.logger.debug('ember packet');
        this.emit(S101CodecEvent.EMBER_PACKET, packet.toBuffer());
    }

    private handleEmberFrame(frame: SmartBuffer): void {
        ...
        if (version !== VERSION) {
            this?.logger.warn('Unknown ember frame version %d', version);
        }

        if (dtd !== DTD_GLOW) {
            this?.logger.error('Dropping frame with non-Glow DTD');
            return;
        }

        if (appBytes < 2) {
            this?.logger.warn('Frame missing Glow DTD version');
            frame.skip(appBytes);
        } else {
            // minor
            frame.readUInt8();
            ...
            if (appBytes > 0) {
                frame.skip(appBytes);
                this?.logger.warn('App bytes with unknown meaning left over');
            }
        }
        ...
        if (flags & FLAG_FIRST_MULTI_PACKET) {
            this?.logger.debug('multi ember packet start');
            this.emberbuf.clear();
        }
        ...
        if (flags & FLAG_LAST_MULTI_PACKET) {
            this?.logger.debug('multi ember packet end');
    ...
    private handleFrame(frame: SmartBuffer): void {
        if (!S101Codec.validateFrame(frame.toBuffer())) {
            this?.logger.error('dropping frame of length %d with invalid CRC', frame.length, frame.toString('hex'));
            return;
        }
        ...
        if (slot !== SLOT || message !== MSG_EMBER) {
            this?.logger.error('dropping frame of length %d (not an ember frame; slot=%d, msg=%d)',
